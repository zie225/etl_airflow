...
pour ingester les données, nous Créons un pipeline Extract Transform Load(ETL) à l'aide de python et automatiser avec airflow, prefect, kafka ou le deployer avec gitlab,github action,Jenkins pour le versioning.

Nous avons le fichier pollution_test.ipynb qui ne passe pas par la phase de l'automatisation de l'l'ingestion, une des raisons pour laquelle nous utilisons le pipeline etl à l'aide de script python pour rendre le pipeline scalable et maintainable.
